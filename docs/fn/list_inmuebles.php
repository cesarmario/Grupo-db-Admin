...
while ($inmuebles = mysqli_fetch_assoc($rtsinmuebles)) {
    ...
    if ($inmuebles['plantasInmueble'] > 0) {
        $plantasInmueble = $inmuebles['plantasInmueble'];
    } else {
        $plantasInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['cloacaInmueble'])) {
        $cloacaInmueble = $inmuebles['cloacaInmueble'];
    } else {
        $cloacaInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['gasNaturalInmueble'])) {
        $gasNaturalInmueble = $inmuebles['gasNaturalInmueble'];
    } else {
        $gasNaturalInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['pavimentoInmueble'])) {
        $pavimentoInmueble = $inmuebles['pavimentoInmueble'];
    } else {
        $pavimentoInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['tipoAguaCalienteInmueble'])) {
        $tipoAguaCalienteInmueble = $inmuebles['tipoAguaCalienteInmueble'];
    } else {
        $tipoAguaCalienteInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['aguaCorrienteInmueble'])) {
        $aguaCorrienteInmueble = $inmuebles['aguaCorrienteInmueble'];
    } else {
        $aguaCorrienteInmueble = "&nbsp;-&nbsp;";
    }
    if ($inmuebles['frenteTerrenoInmueble'] > 0) {
        $frenteTerrenoInmueble = $inmuebles['frenteTerrenoInmueble'] . "m2";
    } else {
        $frenteTerrenoInmueble = "&nbsp;-&nbsp;";
    }
    if ($inmuebles['largoTerrenoInmueble'] > 0) {
        $largoTerrenoInmueble = $inmuebles['largoTerrenoInmueble'] . "m2";
    } else {
        $largoTerrenoInmueble = "&nbsp;-&nbsp;";
    }
    if ($inmuebles['antiguedadInmueble'] > 0) {
        $antiguedadInmueble = $inmuebles['antiguedadInmueble'] . " a&ntilde;o/s";
    } else {
        $antiguedadInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['estadoInmueble'])) {
        $estadoInmueble = $inmuebles['estadoInmueble'];
    } else {
        $estadoInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['mejorasInmueble'])) {
        $mejorasInmueble = $inmuebles['mejorasInmueble'];
    } else {
        $mejorasInmueble = "&nbsp;-&nbsp;";
    }
    if (!empty($inmuebles['cocheraInmueble'])) {
        $cocheraInmueble = $inmuebles['cocheraInmueble'];
    } else {
        $cocheraInmueble = "&nbsp;-&nbsp;";
    }

    if ($inmuebles['superficieCubiertaInmueble'] > 0) {
        $superficieCubiertaInmueble = $inmuebles['superficieCubiertaInmueble'] . "m2";
    } else {
        $superficieCubiertaInmueble = "-";
    }
    if ($inmuebles['superficieTotalInmueble'] > 0) {
        $superficieTotalInmueble = $inmuebles['superficieTotalInmueble'] . $inmuebles['tipoSuperficieTotalInmueble'];
    } else {
